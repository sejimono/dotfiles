...
_LOG_FILE_="$_SCREENSHOT_DIR_/.screenshot.log"
# Color
_FG_COLOR_='#cdd6f4'
_BG_COLOR_='#f5c2e7'
_BG_SIZE_=10
...
function check() {
    if [[ $? -eq 1 && ${PIPESTATUS[0]} -eq 1 ]]; then
        _end_job_=$(date +%s)
        summary $_start_job_ $_end_job_ 'failed' && dunstify -u critical -t 3000 -a "Screenshot Tool" "Screenshot" >> $_LOG_FILE_ 2>&1
        exit 1
    fi
}
...
function get_latest_img() { 
    _LATEST_IMAGE_=$(/bin/ls -th $_SCREENSHOT_DIR_ | grep -vE '.screenshot.png$' | grep -E '.png$' | head -n 1)
...
function convert() {
    _target_file_=$( echo "$_LATEST_IMAGE_" | sed 's/.png/.screenshot.png/g'  )
